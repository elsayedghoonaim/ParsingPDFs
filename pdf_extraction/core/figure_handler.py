...
async def vlm_describe_figures(
    ...
    """
    Concurrently describe all extracted figures across all pages using a shared ClientSession.
    ...
    # Single shared session for all figure description calls (connection pooling)
        ...
        async def describe_one(page_idx: int, fig_idx: int, image_bytes: bytes) -> tuple[int, int, str]:
            ...
            async with sem:
                try:
                    ...
                    async with vlm_calls_lock:
                        vlm_calls += 1  # Count the attempted call
    ...
    logger.info(f"Described {len(results)} figures across {len(page_results)} pages ({vlm_calls} VLM calls)")
